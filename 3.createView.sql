...
SELECT
    ROW_NUMBER() OVER (ORDER BY p.elo DESC) as rank,p.name, p.race, ROUND(p.elo),
    ROUND((CASE WHEN (p.win>0)
          ...
          ELSE
               0
     END))as winrate,
     p.win, p.lose, p.social, p.prev_rank, p.player_id, ((ROW_NUMBER() OVER (ORDER BY p.elo DESC)) - p.prev_rank) as rankchange
FROM players p;
